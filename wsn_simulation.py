...
SCENARIO_NAMES = ["100% Aktif", "30% Duty", "10% Duty"]
...
class EnergyGraph:
    ...
    def draw(self, surf, font_xs, active_idx, x, y, w, h):
        ...
        surf.blit(lbl, (x+7, y+5))

        for i, (col, nm) in enumerate(zip(SC_COLORS, SCENARIO_NAMES)):
            lx = x + 7 + i*75
            pygame.draw.line(surf, col, (lx, y+19), (lx+12, y+19), 2)
            t = font_xs.render(nm, True, col)
            surf.blit(t, (lx+14, y+13))

        gx = x+7; gy = y+28; gw = w-14; gh = h-36
        for gi in range(1, 4):
            ly = gy + int(gi/4*gh)
...
class ReportScreen:
    ...
    def draw(self, surf, fonts, results, graph, num_nodes, init_e):
        ...
        t=font_s.render(
            f"Dugum: {num_nodes}   Enerji: {int(init_e)}/dugum   "
            f"Relay esigi: %{int(RELAY_THRESHOLD*100)}   "
            f"Bitis kriterleri: tam olum / teslim<%{int(DELIVERY_MIN*100)} / "
            f"canli dugum<%{int(ALIVE_MIN_RATIO*100)}",
            True, GRAY)
        surf.blit(t,(rx+16,ry+44))

        COL_X = [rx+16, rx+160, rx+278, rx+368, rx+458, rx+548, rx+650, rx+760]
        HDRS  = ["Senaryo","FND","Omur","Teslim","Kayip","Verimlilik","Bitis Sebebi"]
        ty = ry+70
        ...
        lost_best, lost_worst = best_worst("lost",       higher_better=False)
        ...
        for i,(nm,sc_col) in enumerate(zip(SCENARIO_NAMES,SC_COLORS)):
            ...
            t=font_s.render(nm,True,sc_col)
            surf.blit(t,(COL_X[0]+8,ty+6))

            vals_cols=[
                (str(fnd),  cell_col(i,fnd_best,fnd_worst)),
                (str(life), cell_col(i,life_best,life_worst)),
                (str(delv), cell_col(i,delv_best,delv_worst)),
                (str(lost), cell_col(i,lost_best,lost_worst)),
        ...
        gx2=rx+16; gy2=ty+16; gw2=RW-32; gh2=RH-(ty+16-ry)-52
        ...
        for col,lbl in leg_items:
            if col!=WHITE:
                sample=pygame.Rect(lx2,gy2-18,14,14)
                pygame.draw.rect(surf,GREEN_DIM if col==GREEN else RED_DIM,sample,border_radius=3)
            t=font_xs.render(lbl,True,col); surf.blit(t,(lx2+18,gy2-18)); lx2+=t.get_width()+40
...
def main():
    ...
    while True:
        ...
        t=font_l.render("Enerji Farkindalikli KSA Simulasyonu",True,ACCENT_CYAN)
        screen.blit(t,(TOPO_X,12))
        t=font_s.render(f"Modifiye Dijkstra (E^2)  |  Relay esigi %{int(RELAY_THRESHOLD*100)}"
                        f"  |  Seed: {sim.seed}",True,GRAY)
        screen.blit(t,(TOPO_X,40))
        ...
        legend=[("Yuksek",GREEN),("Orta",YELLOW),("Dusuk",ORANGE),("Kritik",RED),
                ("Esik Alti",LOW_E_COL),("Olu",DEAD_COLOR),("Kill",KILLED_COLOR),("Izole",ISOLATED_COL)]
        lx=TOPO_X
        for txt,col in legend:
            ...
            lx+=t_.get_width()+22
        hint=font_xs.render(
            "SPACE:Baslat/Dur  R:Yeni Harita  1/2/3:Senaryo  Shift+Tikla:Kill  ESC:Kapat",
            True,DARK_GRAY)
